...
function openSidebarGroupMenu(node, x, y){
  ...
  add("+Tier 새 티어표", () => {
    if (typeof newTierScratchInFolder === "function") newTierScratchInFolder(node.newPythonContext);
  });
  add("+Brk 새 대진표", () => {
  ...
  add("+Trip 새 여행일지", () => {
    if (typeof newTripScratchInFolder === "function") newTripScratchInFolder(node.newPythonContext);
  });
...
function isTierSearchable(doc){
  return !!(doc && doc.tierDoc && Array.isArray(doc.tierDoc.items));
}
function tierCardsSearchText(doc){
  return isTierSearchable(doc) && typeof tierSearchText === "function" ? tierSearchText(doc.tierDoc) : null;
}
// .bracket 대진표 — 제목·참가자 이름만(사진 바이트는 빼고)
function isBracketSearchable(doc){
  return !!(doc && doc.bracketDoc && Array.isArray(doc.bracketDoc.entries));
}
