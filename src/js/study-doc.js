...
function mountStudyEditor(doc){
  ...
  function render(){
    ...
    visible.forEach((card, index) => {
      ...
      item.onclick = event => { selectedId = card.id; if (event.target.closest(".study-card-edit")) openCardDialog(card.id); else render(); }; item.ondblclick = event => { if (!event.target.closest("button")) openCardDialog(card.id); }; list.appendChild(item);
    });
  ...
  function openCardDialog(id){
    ...
    body.querySelector(".sc-cancel").onclick = ui.dispose; const del = body.querySelector(".sc-delete"); del.hidden = !current; del.onclick = () => { if (!confirm("이 카드를 삭제할까요?")) return; model.cards = model.cards.filter(card => card.id !== current.id); selectedId = ""; ui.dispose(); history.commit(); touch(); render(); };
  ...
  function startSession(){
    let deck = studyFilterCards(model.cards, filter.value); if (shuffleCheck.querySelector("input").checked) deck = studyShuffle(deck); if (!deck.length){ if (typeof toast === "function") toast("고른 조건에 학습할 카드가 없어요.", 2600); return; }
  ...
  const keydown = event => { if (doc.el.hidden || (event.target.closest && event.target.closest("input,textarea,select,[contenteditable=true]"))) return; const key = String(event.key || "").toLowerCase(); if ((event.ctrlKey || event.metaKey) && key === "z"){ event.preventDefault(); event.shiftKey ? history.redo() : history.undo(); } else if ((event.ctrlKey || event.metaKey) && key === "y"){ event.preventDefault(); history.redo(); } else if (event.key === "Delete" && selectedId) openCardDialog(selectedId); };
