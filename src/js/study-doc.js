...
/* icon 을 주면 공용 SVG 를 넣는다. 글자는 반드시 따로 span 에 담는다 — 저장 버튼(.run-save)은
   documents.js 가 원본/사본 배지에 맞춰 글자를 갈아 끼우는데, textContent 로 통째로 쓰면 아이콘까지
   지워지기 때문이다(labelCls 로 .run-save-label 을 달면 그 칸만 바뀐다). */
function studyButton(label, title, className, icon, labelCls){
  const button = document.createElement("button"); button.type = "button"; button.className = className || "study-btn";
  if (icon && typeof window.uiIcon === "function"){
    ...
    else button.classList.add("study-ico");                // 아이콘만 있는 버튼은 정사각형
  } else button.textContent = label;
  if (title){ button.title = title; button.setAttribute("aria-label", title); }   // 아이콘만 있는 버튼을 읽어 줄 이름
  return button;
}
...
function mountStudyEditor(doc){
  ...
  moreBtn.onclick = () => {
    if (typeof MNContextMenu === "undefined"){ csvInput.click(); return; }   // 모듈이 없으면 최소한 들이기라도
