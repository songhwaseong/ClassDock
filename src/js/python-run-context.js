...
function buildPythonDiagnosticHarness(source, fileName){
  ...
  return [
    "import ast as __md_ast, base64 as __md_b64, builtins as __md_builtins, io as __md_io, json as __md_json, tokenize as __md_tokenize, warnings as __md_warnings",
    ...
    "    _md_lines = __md_source.splitlines()",
    "    _md_tokens = []",
    "    try:",
    "        for _md_tok in __md_tokenize.generate_tokens(__md_io.StringIO(__md_source).readline):",
    "            if _md_tok.type == __md_tokenize.NAME: _md_tokens.append(_md_tok)",
    "    except (__md_tokenize.TokenError, IndentationError): pass",
    "    def _md_u16(value): return len(value.encode('utf-16-le')) // 2",
    "    def _md_ast_col(node):",
    "        line = getattr(node, 'lineno', 1); col = max(0, getattr(node, 'col_offset', 0) or 0)",
    "        raw = (_md_lines[line - 1] if 0 < line <= len(_md_lines) else '').encode('utf-8')",
    "        return len(raw[:col].decode('utf-8', 'ignore'))",
    "    def _md_decl_location(name, node):",
    "        start_line = max(1, getattr(node, 'lineno', 1) or 1); end_line = max(start_line, getattr(node, 'end_lineno', start_line) or start_line)",
    "        start_col = _md_ast_col(node)",
    "        hits = [tok for tok in _md_tokens if tok.string == name and start_line <= tok.start[0] <= end_line and (tok.start[0] > start_line or tok.start[1] >= start_col)]",
    "        if not hits: return None",
    "        tok = min(hits, key=lambda item: (item.start[0], item.start[1]))",
    "        line_text = _md_lines[tok.start[0] - 1] if 0 < tok.start[0] <= len(_md_lines) else ''",
    "        return {'line': tok.start[0], 'column': _md_u16(line_text[:tok.start[1]]), 'length': _md_u16(tok.string), 'name': tok.string}",
    "    def _md_arg_loc(node, name):",
    "        if not name or getattr(node, 'lineno', None) is None: return None",
    "        return _md_decl_location(name, node)",
    "    class _md_Scope:",
    "        def __init__(self, kind, parent=None):",
    "            self.kind = kind; self.parent = parent; self.defs = {}; self.loads = []; self.used = set(); self.globals = set(); self.nonlocals = set(); self.children = []",
    "            if parent is not None: parent.children.append(self)",
    "    class _md_Unused(__md_ast.NodeVisitor):",
    "        def __init__(self): self.root = _md_Scope('module'); self.scope = self.root; self.scopes = [self.root]; self.pending_nonlocals = []",
    "        def _child(self, kind):",
    "            child = _md_Scope(kind, self.scope); self.scopes.append(child); return child",
    "        def _add_def(self, name, node, kind='variable'):",
    "            if not name: return",
    "            location = _md_decl_location(name, node)",
    "            if location is None: return",
    "            location['kind'] = kind",
    "            if name in self.scope.nonlocals: self.pending_nonlocals.append((self.scope, name, location)); return",
    "            target = self.root if name in self.scope.globals else self.scope",
    "            target.defs.setdefault(name, []).append(location)",
    "        def _add_load(self, name):",
    "            if name: self.scope.loads.append(name)",
    "        def visit_Global(self, node): self.scope.globals.update(node.names)",
    "        def visit_Nonlocal(self, node): self.scope.nonlocals.update(node.names)",
    "        def visit_Name(self, node):",
    "            if isinstance(node.ctx, _md_ast.Store): self._add_def(node.id, node)",
    "            elif isinstance(node.ctx, _md_ast.Load): self._add_load(node.id)",
    "        def visit_arg(self, node): self._add_def(node.arg, node, 'parameter')",
    ...
    "                if arg is not None and arg.annotation is not None: self.visit(arg.annotation)",
    "        def _define_args(self, args):",
    "            for arg in list(getattr(args, 'posonlyargs', [])) + list(args.args) + list(args.kwonlyargs): self.visit_arg(arg)",
    "            if args.vararg is not None: self.visit_arg(args.vararg)",
    "            if args.kwarg is not None: self.visit_arg(args.kwarg)",
    "        def _visit_function(self, node):",
    "            self._add_def(node.name, node, 'function')",
    "            for item in node.decorator_list: self.visit(item)",
    "            for item in list(node.args.defaults) + [value for value in node.args.kw_defaults if value is not None]: self.visit(item)",
    ...
    "            parent = self.scope; self.scope = self._child('function'); self._define_args(node.args)",
    ...
    "            for item in list(node.args.defaults) + [value for value in node.args.kw_defaults if value is not None]: self.visit(item)",
    "            self._visit_arg_annotations(node.args)",
    "            parent = self.scope; self.scope = self._child('lambda'); self._define_args(node.args); self.visit(node.body); self.scope = parent",
    "        def visit_ClassDef(self, node):",
    "            self._add_def(node.name, node, 'class')",
    "            for item in list(node.decorator_list) + list(node.bases) + [kw.value for kw in node.keywords]: self.visit(item)",
    "            for item in getattr(node, 'type_params', []): self.visit(item)",
    "            parent = self.scope; self.scope = self._child('class')",
    "            for item in node.body: self.visit(item)",
    "            self.scope = parent",
    "        def visit_Import(self, node):",
    "            for alias in node.names: self._add_def(alias.asname or alias.name.split('.')[0], alias, 'import')",
    "        def visit_ImportFrom(self, node):",
    "            for alias in node.names:",
    "                if alias.name != '*': self._add_def(alias.asname or alias.name, alias, 'import')",
    "        def visit_ExceptHandler(self, node):",
    "            if node.type is not None: self.visit(node.type)",
    "            if isinstance(node.name, str): self._add_def(node.name, node, 'exception')",
    "            for item in node.body: self.visit(item)",
    "        def visit_AugAssign(self, node):",
    "            if isinstance(node.target, _md_ast.Name): self._add_def(node.target.id, node.target); self._add_load(node.target.id)",
    "            else: self.visit(node.target)",
    "            self.visit(node.value)",
    "        def visit_NamedExpr(self, node):",
    "            self.visit(node.value)",
    "            if isinstance(node.target, _md_ast.Name) and self.scope.kind == 'comprehension':",
    "                current = self.scope; target = current.parent",
    "                while target is not None and target.kind == 'comprehension': target = target.parent",
    "                self.scope = target or current; self._add_def(node.target.id, node.target); self.scope = current",
    "            else: self.visit(node.target)",
    "        def visit_MatchAs(self, node):",
    "            if node.pattern is not None: self.visit(node.pattern)",
    "            if node.name: self._add_def(node.name, node, 'pattern')",
    "        def visit_MatchStar(self, node):",
    "            if node.name: self._add_def(node.name, node, 'pattern')",
    "        def visit_MatchMapping(self, node):",
    "            for item in node.keys + node.patterns: self.visit(item)",
    "            if node.rest: self._add_def(node.rest, node, 'pattern')",
    "        def _visit_comp(self, node, values):",
    "            if not node.generators:",
    "                for value in values: self.visit(value)",
    "                return",
    "            first = node.generators[0]; self.visit(first.iter)",
    "            parent = self.scope; self.scope = self._child('comprehension'); self.visit(first.target)",
    "            for item in first.ifs: self.visit(item)",
    "            for gen in node.generators[1:]:",
    "                self.visit(gen.iter); self.visit(gen.target)",
    "                for item in gen.ifs: self.visit(item)",
    "            for value in values: self.visit(value)",
    "            self.scope = parent",
    ...
    "    _md_unused_visitor = _md_Unused(); _md_unused_visitor.visit(__md_tree)",
    "    for _md_from, _md_name, _md_location in _md_unused_visitor.pending_nonlocals:",
    "        _md_target = _md_from.parent",
    "        while _md_target is not None and (_md_target.kind == 'class' or _md_name not in _md_target.defs): _md_target = _md_target.parent",
    "        if _md_target is not None: _md_target.defs.setdefault(_md_name, []).append(_md_location)",
    "    def _md_resolve(scope, name):",
    "        if name in scope.globals: return _md_unused_visitor.root if name in _md_unused_visitor.root.defs else None",
    "        if name in scope.nonlocals:",
    "            cur = scope.parent",
    "            while cur is not None and cur.kind != 'module':",
    "                if cur.kind != 'class' and name in cur.defs: return cur",
    "                cur = cur.parent",
    "            return None",
    "        if name in scope.defs: return scope",
    "        child = scope; cur = scope.parent",
    "        while cur is not None:",
    "            if not (cur.kind == 'class' and child.kind in ('function', 'lambda', 'comprehension')) and name in cur.defs: return cur",
    "            child = cur; cur = cur.parent",
    "        return None",
    "    for _md_scope in _md_unused_visitor.scopes:",
    "        for _md_name in _md_scope.loads:",
    "            _md_target = _md_resolve(_md_scope, _md_name)",
    "            if _md_target is not None: _md_target.used.add(_md_name)",
    "    __md_unused = []",
    "    __md_params = []",
    "    for _md_scope in _md_unused_visitor.scopes:",
    "        if _md_scope.kind == 'class': continue",
    "        for _md_name, _md_decls in _md_scope.defs.items():",
    "            if _md_name in _md_scope.used or _md_name.startswith('_') or _md_name in ('self', 'cls'): continue",
    "            __md_unused.extend(_md_decls)",
    ...
    "        if isinstance(__md_node, __md_ast.keyword) and __md_node.arg is not None:",
    "            __md_ploc = _md_arg_loc(__md_node, __md_node.arg)",
    "            if __md_ploc is not None: __md_params.append(__md_ploc)",
    "        elif isinstance(__md_node, __md_ast.arg):",
    "            __md_ploc = _md_arg_loc(__md_node, __md_node.arg)",
    "            if __md_ploc is not None: __md_params.append(__md_ploc)",
    ...
    "__md_payload = __md_json.dumps({'diagnostics': __md_items[:100], 'unusedReady': __md_tree is not None, 'unused': (__md_unused[:500] if __md_tree is not None else []), 'params': (__md_params[:600] if __md_tree is not None else [])}, ensure_ascii=False).encode('utf-8')",
