...
| `trip.js` | 여행일지 문서(`.trip`)의 모델과 형식입니다. 파일 하나가 여행 한 건이고 속은 일기장과 같은 무압축 ZIP(`trip.json` + `assets/*`)이라, ZIP·해시·사진 줄이기·스티커 정규화는 `diary.js` 것을 그대로 부릅니다. **갈래(purpose) 셋**(개인 여행 기록 `trip` · 체험학습 학습지 `field` · 사회과 답사 보고서 `survey`)은 자료를 가르지 않고 부르는 말·기본값·접기만 바꿉니다 — 낱말은 `TRIP_WORDS`/`TRIP_WORDS_EN` 한 표에 모으고(빈 문자열 = 그 갈래에서 감춤, `{n}`·`{sum}` 이 있는 줄은 낱말이 아니라 틀이라 `tripWordf` 로 채웁니다), 장소의 종류는 세 갈래의 **합집합 13가지**(`TRIP_SPOT_KINDS`)로 두고 고르개에 무엇을 보일지만 갈래가 정합니다. **모르는 종류도 버리지 않습니다** — 갈래를 바꿨다고 값이 사라지면 무손실 규칙이 깨지기 때문입니다(일기장이 모르는 `art` 를 버리는 것과 반대 규칙). 하루(day)는 일기장의 하루와 같은 모양에 장소(spots)가 얹힌 것이고, 장소를 스티커와 따로 두는 까닭은 지도·일정·경비 세 곳이 같은 자료를 읽기 때문입니다. 국내·해외 판정은 `tripIsDomestic` 한 곳에서만 합니다(좌표가 하나도 없으면 국내로 봅니다). 설계는 `docs/여행일지-설계.md`. | `diary.js`(종이 엔진 `mountDiaryPaper`·ZIP·스티커), `korea-coords.js`, `documents.js`, `file-loaders.js`, `tests/trip.test.js` |
