...
| `tier-list.js` | 티어표 문서(`.tier`)의 등급 줄·카드 모델과 편집 화면입니다. 카드 차례는 `items` 배열 순서이고 `tier` 가 빈 카드가 아래 모음입니다. 포인터 이벤트로 짠 끌어 옮기기(마우스·펜·터치 공용), 사진 여러 장·끌어다 놓기·붙여 넣기(작게 줄여 data URL), 줄 설정·틀 6종·카드 크기, 숫자 키 줄 보내기, 캔버스로 직접 그리는 PNG 저장과 칠판 보내기를 담당합니다. 되돌리기 기록엔 사진 대신 짧은 열쇠만 담습니다. |
