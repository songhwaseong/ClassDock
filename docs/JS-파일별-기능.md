...
| `jeju-bus-api.js` | TAGO(국가대중교통정보센터) 응답 봉투 해석과 런처 조회. 결과 코드·단건/빈 결과 모양, 노선 ID, 좌표 검증, 원본 수신시각과 캐시 지연 정보, 키·한도 오류 까닭을 보존합니다. | `jeju-bus-map.js`, `desktop/launcher.cs`, `tests/jeju-bus-live.test.js` |
...
| `jeju-bus-map.js` | 제주 버스 패널, 노선 선택, Leaflet 차량·노선 층, 숨김·종료 시 요청 정리, 캡처 시 위치 고정을 담당합니다. 문서 모델에는 기록하지 않습니다. | `map-viewer.js`, `tests/jeju-bus-controller.test.js` |
