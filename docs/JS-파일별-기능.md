...
| `workspaces.js` | 한 창 안의 가상 작업공간 목록과 전환 UI(문서 탭 줄 왼쪽 `#workspaceMenuBtn` 버튼 하나가 목록·새로 만들기·이름 변경·삭제 메뉴를 엽니다. 탭이 0개여도 `#tabBar` 를 숨기지 않고, `renderTabs()` 는 `#docTabs` 에만 그립니다), 작업공간별 문서·탭·활성 문서·분할 배치·사이드바 상태를 저장·복원합니다. 같은 원본은 문서 모델 하나를 여러 작업공간이 공유해 저장 충돌을 막습니다. 빈 화면 판정(`workspaceActiveNodes`)은 사이드바가 실제로 그릴 수 있는 줄(루트까지 이어지고 문서가 살아 있는 줄)만 세고, 복원 때 빈 하위 폴더의 소속은 부모를 따릅니다. | `state.js`, `documents.js`, `file-loaders.js`, `workspace-store.js` |
...
| `hand-font-pen.js`·`hand-font-brush.js` | 일기장 손글씨 글꼴(나눔손글씨 펜·붓, SIL OFL 1.1) WOFF2 base64 데이터입니다. 자동 생성 파일(`tools/build-hand-fonts.py`)이며 라이선스는 `vendor/licenses/nanum-handwriting-OFL.txt`. 일기장에서 그 글꼴을 고를 때만 지연 로드합니다. | `diary.js`, `lazy.js`(`handPen`·`handBrush`) |
| `hand-font-hippie.js`·`hand-font-dahaeng.js`·`hand-font-student.js`·`hand-font-amsterdam.js`·`hand-font-mago.js` | 일기장 손글씨 글꼴 다섯 벌(나눔손글씨 모음 CLOVA — 바른히피·다행체·중학생·암스테르담·마고체, SIL OFL 1.1). 자주 쓰는 한글 2,350자만 남기고 글꼴 이름을 `ClassDock Hand …` 로 바꾼 고친 글꼴입니다(빠진 글자는 펜이 이어 그림). 자동 생성 파일(`tools/build-hand-fonts.py`)이며 라이선스는 `vendor/licenses/nanum-clova-handwriting-OFL.txt`. 고를 때만 지연 로드합니다. | `diary.js`, `lazy.js`(`handHippie` 등) |
| `hand-font-samples.js` | 꾸미기 창 글꼴 칩 견본용 — 손글씨마다 "가나다" 세 글자만 남긴 작은 WOFF2(약 15KB). 창을 열 때 지연 로드합니다. | `diary.js`, `lazy.js`(`handSamples`) |
...
| `korea-coords.js` | 한국 평면 좌표계(UTM-K 5179 · 중부원점 GRS80 5186/5181 · 중부원점 Bessel 2097/5174 · 서부·동부·동해원점 · UTM 52N · 카카오맵 좌표) → WGS84 위경도 변환 순수 모듈(`MNKoreaCoords`)입니다. 지도 표 들이기가 위경도 대신 평면 좌표가 적힌 줄을 버리지 않고 바꿔 넣습니다. 카카오 좌표 변환 API 는 UTM-K 를 받지 않아(400) 쓰지 않고 횡메르카토르(크뤼거 4차)와 한국측지계 1985 → WGS84 7변수 변환을 직접 계산합니다 — 인터넷·키·호출 한도가 필요 없습니다. 결과는 카카오 변환값과 1m 안으로 맞는지 테스트로 붙들어 둡니다(카카오 'TM' 은 2097 이 아니라 10.405초 보정된 5174 와 같습니다). | `map-viewer.js`(표 들이기), `tests/korea-coords.test.js` |
| `flight-api.js` | 한국공항공사 '실시간 항공기 운항정보 조회·검색' 응답 해석(`MNFlightApi`). 좌표가 오지 않아 공항 코드(IATA)→좌표 표(국내 15곳·국제선 상대 공항)를 들고 있고, 상태 글을 갈래(예정·수속·탑승·출발·도착·지연·결항·회항)로 묶습니다. **한 쪽 100줄이 한계**라(넘기면 HTTP 200 에 게이트웨이 오류 04) 큰 게시판은 쪽을 넘겨 묻습니다. 자정 넘긴 변경 시각, 빈 상태(아직 안 움직인 편)도 여기서 처리합니다. DOM 없는 모듈이라 `node --test` 로 검증합니다. | `flight-map.js`, `desktop/launcher.cs`(`/flight-board`·`/flight-search`), `tests/flight-api.test.js` |
| `flight-map.js` | 지도 '✈ 항공' 패널과 노선 층(`MNFlightMap`). 공항·출발/도착·국내/국제 게시판을 받아 상대 공항마다 대권 곡선 하나(굵기=편수, 색=지연·결항)로 긋고, 목록·상대 공항 칩·편명 찾기를 보입니다. 비행기 위치는 쫓지 않고, 문서 모델에 쓰지 않으며, 몇 분마다 새로 받습니다. | `map-viewer.js` |
| `ship-ports-data.js` | 여객선 기항지 이름 → 좌표 표(`MNShipPortData`). TAGO 시간표에 좌표가 없어 2026-09-19 하루치 전국 시간표(항구 333곳·이름 345개)의 이름을 OpenStreetMap(Nominatim)으로 찾아 만들었습니다(© OpenStreetMap contributors, ODbL). 같은 이름 항구가 둘이면 두 자리를 둡니다. 새 기항지가 생기면 이 표에 더합니다. | `ship-api.js`, `tests/ship-api.test.js` |
| `ship-api.js` | TAGO 국내선박운항정보(여객선 시간표) 응답 해석과 런처 조회(`MNShipApi`). 선박명 공백 겹침·운임 0·단건/빈 결과를 정리하고, 한 배가 같은 시각에 여러 섬을 들르는 편을 한 항해로 묶습니다(도착 시각은 모든 기항지에 같은 값이 와서 소요 시간으로 읽지 않습니다). 같은 이름 항구는 출발 항구에 가까운 자리를 고릅니다. | `ship-map.js`, `desktop/launcher.cs`(`/ship-ports`·`/ship-schedule`), `tests/ship-api.test.js` |
| `ship-map.js` | 지도 '여객선' 패널과 뱃길 층(`MNShipMap`). 출발 항구·날짜를 골라 시간표를 받고, 도착지마다 점선(실제 뱃길이 아님)을 긋고, 항해 목록·도착지 칩·지도 가운데 근처 항구 찾기를 보입니다. 결항·실시간 위치는 없다고 패널에 밝힙니다. 문서 모델에는 쓰지 않습니다. | `map-viewer.js` |
| `weather-api.js` | 기상청 날씨(단기예보·초단기실황·지상관측 일자료)와 한국천문연구원 특일(공휴일·24절기) 응답 해석과 런처 조회(`MNWeatherApi`). 위경도→기상청 격자(nx,ny) 변환, 종관기상관측 지점 97곳 표(가장 가까운 지점 고르기), 하늘·강수 형태 → 일기장 날씨 값 바꾸기, 특일 달별 캐시(이 브라우저에 일주일)를 맡습니다. 빈 값("")은 0 이 아니라 '없음'으로 읽습니다. | `weather-map.js`, `diary.js`, `desktop/launcher.cs`(`/weather-*`), `tests/weather-api.test.js` |
| `weather-map.js` | 지도 '날씨' 패널과 딱지 층(`MNWeatherMap`). 지도 가운데의 지금 날씨·시간별 예보·모레까지 최고/최저, 전국 주요 도시 19곳의 지금 날씨를 지도 위 딱지로 보입니다. 문서 모델에는 쓰지 않습니다. | `map-viewer.js` |
| `kosis-api.js` | KOSIS 국가통계포털 자주 쓰는 통계 36개 목록과 응답 해석·런처 조회(`MNKosisApi`). 표마다 다른 지역 코드(11010·11110·11101HJG11010·1236010)를 끝 숫자 앞머리로 시도에 잇고, 일반구에는 시를 끼워 "경기도 수원시 장안구" 꼴로 만듭니다. 합계 줄(전국·동부·통합청주시)과 값 없음("-")은 뺍니다. | `kosis-choro.js`, `desktop/launcher.cs`(`/kosis`·`/kosis-key`), `tests/kosis-api.test.js` |
| `kosis-choro.js` | 색칠 지도 창의 'KOSIS에서 가져오기' 칸(`MNKosisChoro`). 자주 쓰는 통계 고르기와 검색→표→항목·분류·해 고르기 두 길로 받아, 색칠 지도 붙여넣기 칸에 "지역	값" 표를 채웁니다. | `map-viewer.js` |
| `neis-api.js` | NEIS 교육정보 개방 포털 — 학교 찾기·급식·학사일정·시간표 응답 해석과 런처 조회(`MNNeisApi`). 급식 메뉴의 `<br/>`·알레르기 번호를 걷어 내고, 학사일정의 휴업일을 가리고(매주 오는 토요휴업일은 뺌), 학교급에 맞는 시간표 서비스(초 els·중 mis·고 his)를 고릅니다. 고른 학교는 이 브라우저(localStorage `mn.neisSchool`)에만 남깁니다. | `diary.js`, `desktop/launcher.cs`(`/neis`·`/neis-key`), `tests/neis-api.test.js` |
| `jeju-bus-map.js` | 버스 패널(전국 도시 고르기), 노선 선택, Leaflet 차량·노선 층, 근처 정류장·도착 안내, 숨김·종료 시 요청 정리, 캡처 시 위치 고정을 담당합니다. 문서 모델에는 기록하지 않습니다. | `map-viewer.js`, `tests/jeju-bus-controller.test.js`, `tests/bus-nationwide.test.js` |
| `subway-stations.js` | 수도권 전철 16개 노선 664개 역의 좌표와 이웃 관계를 담은 **순수 데이터**입니다(로직 없음 — `pet-data.js` 와 같은 자리). 실시간 열차 위치를 지도 위에서 움직이게 하려면 API 가 주지 않는 두 가지가 필요합니다 — 역 좌표와 '다음 역이 어디인가'. 자료는 OpenStreetMap 노선 관계에서 뽑았고(© OpenStreetMap 기여자, ODbL — 화면에 띄울 때 출처를 함께 보여야 합니다) `tools/build-subway-stations.mjs` 로 다시 만듭니다. 키는 실시간 API(`realtimePosition`)의 `statnNm` 을 **공백 제거·괄호 부역명 제거·끝의 '역' 제거**로 다듬은 이름이고, 그 규칙만으로 실제 관측 312건이 모두 붙었습니다. OSM 이 부역명으로 올린 `자양` 은 생성기가 실시간 API 이름인 `뚝섬유원지` 로 바꿔 넣습니다. **`statnId` 로 역 순서를 짐작하면 안 됩니다** — 순서처럼 보이지만 신분당선은 자리수가 늘어 689 다음이 6810 이고, 공항철도는 나중에 끼워 넣은 역이 앞 역 번호에 가지번호를 답니다(마곡나루 65042). 순서는 반드시 이웃(`n`)으로 봅니다. 이웃에는 방향이 없어서 **진행 방향은 '직전에 있던 역이 아닌 쪽'** 으로 정하고, 갈래가 둘 이상인 분기역에서만 종착역으로 고릅니다 — 종착역 최단경로만 쓰면 순환선(2호선)을 먼 쪽으로 도는 열차와 회차 직전이라 종착역이 이미 바뀐 열차에서 틀립니다. 노선 목록은 `main.go`·`launcher.cs` 의 허용 목록과 항상 같아야 합니다(`tests/subway-stations.test.js` 가 세 곳을 함께 검사). | `tools/build-subway-stations.mjs`, `desktop/main.go`(`/subway-position`), `desktop/launcher.cs`(`/subway-position`), `tests/subway-stations.test.js` |
| `subway-live.js` | 실시간 열차 위치 계산 공용 모듈(`MNSubwayLive`)입니다. 실시간 API 는 좌표를 주지 않고 '어느 역에 어떤 상태로'(0 진입·1 도착·2 출발·3 전역출발)라는 이벤트만, 그것도 역을 지날 때 한 번씩 보냅니다. 이 모듈은 그 이벤트를 `subway-stations.js` 의 좌표·이웃 표에 얹어 '지금 두 역 사이 몇 %인가'를 냅니다. DOM·fetch 를 쓰지 않는 순수 모듈이라 `node --test` 로 그대로 검증합니다. **진행 방향은 '직전에 있던 역이 아닌 쪽'** 입니다 — 실측 359건에서 updnLine 만 88.8%, 종착역 최단경로 98.1%, 직전 역까지 쓰면 99.4%였습니다. 종착역만 쓰면 순환선(2호선)을 먼 쪽으로 도는 열차와 회차 직전이라 종착역이 이미 바뀐 열차(우이신설선)에서 틀리고, 갈래가 둘 이상인 분기역에서만 종착역으로 고릅니다. **'출발' 이벤트는 도착 뒤 50초쯤에야 찍혀서**, 최신 행만 보고 그리면 같은 구간에서 진행률이 뒤로 밀려 열차가 튑니다 — `positionOf` 가 같은 구간의 이벤트끼리 가장 많이 간 값을 씁니다. 보간 상수(정차 45초 + 거리÷15m/s)는 '도착→도착' 291건으로 맞췄고(표정속도 34km/h), 진행률을 0.97에서 막아 다음 역을 지나치지 않게 하면 위치 오차 중앙값이 39m입니다. **역별 도착 정보**(`arrivals`)도 읽습니다 — 역 점을 누르면 런처 `/subway-arrival` 로 묻는데, 역 이름은 API 원래 이름이어야 해서(`총신대입구`→자료 없음, `총신대입구(이수)`→정상) 노선별 대응표 `API_NAMES`(2026-09-18 전체 역 조회에서 뽑음)로 바꿔 묻습니다. 같은 열차가 받은 시각만 달리 두 줄로 오면 늦은 것만 남기고, `ordkey` 의 차례 자리는 실제 도착 순서와 어긋나 정거장 수로 줄 세웁니다. | `subway-stations.js`, `map-viewer.js`(실시간 층·도착 칸), `tests/subway-live.test.js` |
| `map-viewer.js` | 지도 문서(`.map`)입니다. Leaflet(`vendor/leaflet.min.js`, `MNLazy`의 `leaflet` 묶음)으로 배경지도를 띄우고 표시(마커)를 찍어 이름·메모·색을 붙이고 끌어 옮기며, 좌표로 이동·배경지도 바꾸기(일반·지형·흑백·위성)를 제공합니다. `.map`은 중심 좌표·확대·표시 목록을 담은 JSON이라 통합검색·되돌아 열기와 잘 맞습니다. **카카오·네이버 지도를 쓰지 않은 이유**: 두 SDK 모두 JS 키에 사이트 도메인을 등록해야 하는데 이 앱은 실행마다 127.0.0.1의 빈 포트를 잡고 오프라인 HTML은 `file://`로도 열려 등록할 주소가 없고, 키가 배포본에 실려 퍼지며 타일 저장도 약관이 막습니다. 라이브러리는 vendor에 있어 오프라인이고 인터넷은 배경 타일에만 필요합니다. exe로 돌 때는 배경 타일을 런처의 `/tile-proxy`로 받아 **화면에 실제로 표시된 타일만** 서버 디스크에 자동 캐시하며(계속 실패하면 조용히 직접 주소로 되돌립니다), 공개 지도 서버가 금지하는 보지 않은 범위·확대 단계의 사전 다운로드는 하지 않습니다. 배경지도 호스트는 `launcher.cs`의 `TileProxyHosts` 허용 목록과 항상 같아야 합니다(`tests/map-viewer.test.js`가 검사). **`🖊️ 칠판으로`**는 지금 보이는 지도를 PNG 로 굳혀 새 화이트보드에 올립니다 — 타일이 다 들어오길 기다린 뒤 확대·이동 단추와 말풍선·이름표 칸을 감추고(닫은 말풍선은 페이드아웃 때문에 200ms 가량 DOM 에 남아 그냥 찍으면 편집 서식이 지도 한복판에 박힙니다) `htmlToImage` 로 찍고, 캔버스에서 표시 이름과 출처를 그림에 새겨 넣습니다(정지 그림이라 말풍선으로는 이름이 남지 않고, OSM 라이선스상 출처도 함께 있어야 합니다). 칠판은 빈 스냅샷을 지금 시각으로 넘겨 열어 같은 이름으로 쓰던 옛 판서가 되살아나지 않게 하고, 렌더 뒤 `doc.insertBoardImage` 로 그림을 넣습니다. 지도 문서는 그대로 남습니다. **`openMapPicker()`**는 반대 방향입니다 — 칠판의 `🗺️` 가 부르는 지도 고르기 창으로, 자리를 잡아 '이 화면 넣기'를 누르면 캡처한 PNG 를 돌려줍니다(지도 문서를 만들지 않고 그림만 가져가는 길이며, 마지막 자리를 기억합니다). 검색칸은 **검색 버튼과 Enter를 같은 실행 경로로 연결**하고 프록시 확인보다 먼저 이벤트를 붙여 창을 연 직후에도 입력을 놓치지 않습니다. **`🗂️ 오프라인 지도`**는 자동 캐시 현황과 400MB 상한·비우기를 보여 주며 프록시가 있을 때(=런처로 실행)만 붙습니다. 프록시 유무는 `/can-proxy-tiles` 로 확인합니다(저장 가능 여부와 다른 능력이라 프로브를 따로 둡니다 — Go 폴백 런처는 저장은 못 해도 타일은 받습니다). **장소 이름 검색**(`mapGeocode`·`mapAttachPlaceSearch`)은 런처의 `/geocode`만 거칩니다. 런처가 식별 User-Agent·초당 1건 제한·검색 캐시를 적용하고 `CLASSDOCK_GEOCODER_URL`로 Nominatim 호환 HTTPS 공급자를 바꿀 수 있습니다. `file://`에서는 이름 검색을 막고 좌표처럼 생긴 입력(`mapParseCoords`)만 조회 없이 바로 이동합니다. 도구막대는 두 줄(늘 남는 머리말 `.map-bar` + 접히는 `.map-tools`)이라 **`▤ 도구 숨기기`(자판 `H`)** 로 편집 도구만 접어 지도를 넓게 볼 수 있습니다 — 접어도 우클릭 메뉴(`contextMirror(toolsToggleBtn)`)로 되돌아오고, 접은 상태는 `.map` 이 아니라 `localStorage`(`mapToolbarVisible`)에 남습니다. ⛶ 전체화면에서는 머리말까지 임시로 접고 나오면 되돌립니다(`fullscreenchange` + `body.viewer-fullscreen` 클래스 감시 — 창 안 폴백에는 이벤트가 없습니다). 지도 문제(학생) 화면은 두 줄 다 접은 채로 두고 토글 자체를 내놓지 않습니다. **표시 이름 늘 보이기(`🏷️`)** 는 마커 툴팁을 상시 표시로 바꾸되 확대 13단계 미만에서는 접고(이름표가 포개져 못 읽습니다) 표시 200개(`MAP_LABEL_MAX_MARKERS`)를 넘으면 켜지지 않으며, 켠 상태는 `.map`(버전 4)에 저장됩니다 — 칠판·PNG·인쇄 그림에는 이 값과 상관없이 언제나 모든 이름이 새겨집니다. **카카오 연동**(REST 키가 있을 때)은 네 갈래입니다 — `🔎 장소 정보`(누른 좌표 역검색, 확대 15단계 이상), `🏫 주변 시설`(카카오 갈래 18가지 중 **다섯까지**(`MAP_NEARBY_MAX_KINDS`) 동시 검색, 개수는 갈래당이 아니라 **전체 최대**(`MAP_NEARBY_TOTAL_CHOICES` 30·45·75·100, 기본 75)를 갈래끼리 고르게 나누고 한 갈래 45곳(`MAP_NEARBY_MAX_PER_KIND`)을 넘기지 않음, 겹치는 갈래 색은 자동으로 비껴 줌, 넣은 것에는 `source:"nearby"`·`batch` 꼬리표가 붙어 되돌리기·🧹 지우기가 묶음째 뭅니다), `🗺 카카오맵 상세 보기`(`openMapKakaoPlaceModal` — `place.map.kakao.com/<숫자>` 만 통과시키는 `mapKakaoPlaceUrl` 로 URL을 다시 검사하고, **같은 `batch` 로 들어온 시설만** 한 벌로 묶어 iframe **하나를 갈아 끼우며** `‹ · ›`·`←`·`→` 로 순환 이동, 카카오 안내에 따라 페이지를 덮거나 자르지 않습니다), `🚗 자동차 길찾기`(카카오모빌리티 길찾기 API — 유일하게 Local API 가 아닌 `apis-navi.kakaomobility.com` 이지만 키는 Local API 에 쓰던 그 REST 키 하나이고 콘솔에서 따로 켤 제품 설정은 없습니다 — 하루 무료 몫만 이 API 에 따로 매겨집니다. 표시 잇기와 같은 차례를 실제 도로로 이어 주행 거리·예상 시간을 이름표에 적으며, 첫 표시가 출발·마지막이 도착·사이가 경유지라 카카오 상한(경유지 5) 때문에 **표시 7개**(`MAP_DRIVE_MAX_MARKERS`)까지 한 번에 잇고 넘치면 앞쪽만 이은 사실을 이름표에 적습니다. 길은 `.map` 에 담지 않고 켠 사실만 담아(버전 8) 표시를 옮기면 손 뗀 뒤 다시 물으며, 같은 표시 배치로는 두 번 묻지 않게 좌표 서명으로 답을 담아 둡니다). **`🧾 표로 메모`** 는 `window.addTableToScratchpad` 로 표시 정보를 메모 표 블록으로 보내고(메모용은 `mapMarkersToMemoRows` 의 여덟 칸, 저장용 CSV는 `mapMarkersToRows` 로 모든 필드), 상한에 걸려 빠진 줄 수를 받아 알립니다. **거리선·면적 그리기**는 첫 점 이후 마지막 점부터 커서까지 점선 안내선(`draftGuideLayer`)을 그리고, 그리는 중의 **우클릭은 빠른 메뉴 대신 `finishDrawing(true)`**(`Enter`·같은 버튼 다시 누르기와 한 길)이며 점이 모자라면 브라우저 메뉴를 막고 안내만 보여 줍니다. 그 밖에 축척 막대·방위표·`🌐` 위경도 격자, `🧾` 표시 목록과 묶음 감추기, `🎬` 발표 모드, 표시 사진(base64·합계 12MB), CSV 들이기·내보내기와 주소 역검색·지역 통계, `📷` PNG·`🖨️` 인쇄, `📋` 메모 왕복, `🎯` 지도 문제(`.task`) 출제·풀이가 이 파일에 함께 있습니다. **🚇 실시간 열차**(런처가 `/can-proxy-subway` 로 답할 때만 붙습니다)는 고른 노선의 **역·역 사이 선**을 전용 pane(`mapSubwayRoutePane`, z 370 — 사람이 그린 것보다 아래)에 깔고, 그 위 `mapSubwayPane`(z 640)에 열차를 그립니다. 15초마다 `/subway-position` 을 묻고 그 사이는 `MNSubwayLive` 로 이어 그리며(매 프레임), 다른 탭을 보고 있거나 문서가 숨겨져 있으면 아예 묻지 않습니다(하루 1,000회 한도). 인증키가 없으면(428) 이유를 밝히고 **스스로 끕니다**. 이 층은 **`.map` 에 아무것도 저장하지 않습니다** — 켜 둔 노선조차 `localStorage`(`mapSubwayLine`)에만 두는데, 지도를 건네받은 사람에게는 인증키가 없기 때문입니다. **역 이름은 확대 14단계(`MAP_SUBWAY_LABEL_MIN_ZOOM`)부터 permanent 툴팁으로 늘 붙고** 그보다 멀면 접습니다 — 역은 표시보다 촘촘해 13단계에서는 글자가 겹칩니다. Leaflet 은 bindTooltip 뒤에 permanent 를 못 바꿔 다시 매다므로 상태가 뒤집힐 때만 부릅니다. 캡처는 이름표 칸을 통째로 감추므로 화면에 붙어 있는 역 이름은 `subwayCaptureLabels()`(바깥 스코프에 걸어 두는 훅)로 넘겨 그림에 직접 새깁니다. 다만 캡처(칠판·PNG·인쇄)에는 일부러 남깁니다. **🎨 색칠 지도(단계구분도, `.map` v13 `choropleth`)** 는 순수 함수 `mapChoro*`·`openMapChoropleth` 로 표·CSV·엑셀 값이나 표시 개수를 시도·시군구·읍면동 경계에 칠합니다. 경계는 지연 묶음 `koreaRegions`(`vendor/korea-regions.js`, 시도·시군구, 2026-07·2025-12 두 시점)와 `koreaEmd`(`vendor/korea-emd.js`, 읍면동 — 늘 범위를 정하고 코드 우선 매칭)이며 `tools/build-korea-regions.mjs` 로 만듭니다. 출처표시 의무가 있어 캡처 출처 줄·범례에 남기고, 경계층은 `interactive:false`(지도 우클릭 메뉴 보호)라 hover 는 점-다각형 판정으로 합니다. 켜진 🎨 를 다시 누르면 걷히고 범례 클릭으로 설정을 고칩니다. 지도 탭을 닫으면 `cleanupFns`에서 Leaflet·ResizeObserver·전역 키 리스너를 함께 해제합니다. | `lazy.js`, `documents.js`, `file-loaders.js`, `code-viewer.js`, `whiteboard.js`(`insertBoardImage`), `desktop/launcher.cs`(`/tile-proxy`·`/geocode`), `vendor/leaflet.min.js`, `vendor/korea-regions.js`, `vendor/korea-emd.js`, `tests/map-viewer.test.js` |
...
| `tier-list.js` | 티어표 문서(`.tier`)의 등급 줄·카드 모델과 편집 화면입니다. 카드 차례는 `items` 배열 순서이고 `tier` 가 빈 카드가 아래 모음입니다. 포인터 이벤트로 짠 끌어 옮기기(마우스·펜·터치 공용 — 카드도, ⠿ 손잡이로 줄 차례도), 보유 카드 검색(화면에서만 거르고 저장하지 않음), 사진 여러 장·끌어다 놓기·붙여 넣기(작게 줄여 data URL), 줄 설정·틀 6종·카드 크기, 숫자 키 줄 보내기, 월드컵(토너먼트 — `tierCupStart`/`tierCupPick` 순수 함수로 둘 중 하나를 골라 올라가고, 홀수 판은 끝 카드 부전승, 끝나면 진 판 크기대로 순위를 묶어 `tierCupPlace` 로 줄에 놓음. 진행 상태는 파일에 담지 않음), 캔버스로 직접 그리는 PNG 저장과 칠판 보내기(화면과 같은 모양 — 줄 장식 그림은 `TIER_ICON_PATHS` 경로를 SVG·`Path2D` 가 함께 씀. 그림 이름은 줄마다 `icon` 에 담겨 줄을 옮겨도 따라가고, 없는 예전 파일은 읽을 때 차례로 정함)를 담당합니다. 머리말은 판과 같은 1400px 틀(`.tier-bar-inner`)에 두고 판 쪽 스크롤 막대 폭(`--tier-gutter`)만큼 비워 넓은 화면에서도 양 끝이 맞습니다. 머리말 오른쪽 고양이는 장식 SVG(`TIER_MASCOT_SVG`)라 눌리지 않고, 판을 내려 보면 숨습니다. 되돌리기 기록엔 사진 대신 짧은 열쇠만 담습니다. |
| `bracket.js` | 대진표 문서(`.bracket`)의 토너먼트 모델과 편집 화면입니다. 나무는 늘 2의 거듭제곱 칸(2~128)이고 빈 칸은 부전승이며, 1회전 자리는 `slots`(씨드 차례 `bracketSeedOrder` 로 앉혀 부전승이 고루 흩어짐), 경기 결과는 `results["회전:번호"]` 에 두 참가자(a·b)와 함께 담아 앞 경기가 바뀌면 뒤 결과가 저절로 무효가 됩니다(`bracketResolve`·`bracketPrune`). 모양 6가지(양쪽→가운데·왼→오·오→왼·아래→위·위→아래·위아래→가운데)는 `bracketGeometry` 한 곳에서 같은 나무를 줄(lane)×자리(cross)로 계산한 뒤 좌표만 바꿉니다. 이긴 쪽을 누르면 선이 차오르고 카드가 다음 칸으로 날아가는 애니메이션(Web Animations), 우승 왕관·색종이, 정한 경기를 1회전부터 되살리는 **다시 보기**(회전이 바뀌면 그 회전부터 크게), 회전 이름표를 눌러 그 회전부터 우승까지만 크게 보는 **회전별로 크게 보기**(`bracketFocusGeometry` — 작은 대진으로 배치하고 번호는 원래 대진 번호로 되돌려 경기 로직을 그대로 씀. 빠지는 카드는 옛 판 복제 위에서, 남는 카드는 FLIP 으로 옮김), 점수 입력(높은/낮은 점수 승), 배경 템플릿 10종+내 사진(`[data-br-theme]` CSS 변수 — PNG 는 `BRACKET_THEMES.png` 색으로 근사), 꺾은선/곡선·카드 크기, 확대·밀기, 첫 칸 끌어 자리 바꾸기, 캔버스로 직접 그리는 PNG 저장·칠판 보내기, 메모 왕복(그림 + 스냅샷을 `window.addBracketToScratchpad` 로 한 블록에 담고, 메모의 "✏️ 대진표로"가 `openBracketFromMemo` 로 되살림 — `doc.memoBlockId` 로 같은 블록을 제자리 교체, 같은 블록은 한 탭만)를 담당합니다. 사진 줄이기·창(`tierPrepareImage`·`tierModal`)은 `tier-list.js` 것을 함께 씁니다. 되돌리기 기록엔 사진 대신 짧은 열쇠만 담습니다. | `tier-list.js`, `documents.js`, `file-loaders.js`, `history.js`, `code-viewer.js` |
| `pick.js` | 복불복 뽑기 문서(`.pick`)의 참가자 명단과 공통 화면입니다. 파일엔 명단(이름·색·사진·`off`=이번엔 빼기)·고른 게임·게임별 설정(`games`, 모르는 게임 설정은 작은 JSON 이면 그대로 들고 감)만 담고 뽑은 결과는 화면에만 둡니다. 뽑기는 `pickRandomInt`(crypto)로 먼저 정하고 게임은 그 결과에 맞춰 움직임만 보여 줍니다. 게임은 `pick-*.js` 가 `PICK_GAME_IMPL[id] = { mount, normalize }` 로 올리고, 이 파일은 머리말·참가자 목록(추가 입력칸·여러 줄 붙여넣기·사진 끌어다 놓기)·결과 창·뽑힌 차례·효과음(WebAudio)·색종이를 맡습니다. 게임이 도는 동안(`setBusy`)엔 명단을 바꿀 수 없습니다. |
| `pick-roulette.js` | 복불복의 룰렛 돌리기입니다. 바늘은 12시에 고정되고 바퀴가 시계 방향으로 돕니다. 칸 번호는 `pickRouletteIndexAt` 하나로 셈하고, 멈출 각은 뽑힌 칸 안의 한 점(가장자리는 피함)이 바늘 아래 오도록 `pickRouletteTarget` 이 거꾸로 셈합니다. 8칸까지는 이름을 똑바로, 그보다 많으면 반지름 방향으로 눕혀 씁니다. 칸이 바늘을 지날 때마다 딱 소리와 바늘 튕김이 납니다. |
| `pick-ladder.js` | 복불복의 사다리타기입니다. 가로줄은 층마다 이웃 칸이 겹치지 않게 놓아(`pickLadderBuild`) 누구의 길도 갈라지지 않고 위 n 명과 아래 n 칸이 하나씩 짝이 되며, 이웃한 두 줄 사이에는 가로줄이 적어도 하나 있습니다. 길은 `pickLadderTrace`, 아래 칸은 `pickLadderLabels`(당첨 k 칸 또는 칸마다 직접 쓴 글, 차례 섞음)입니다. 가로줄은 탈 때까지 가리고(설정으로 미리 보이기), 모두 타기·이름 눌러 한 명만 타기를 픽셀 좌표 SVG 에 점선 길이로 조금씩 드러내 그립니다. 20명까지입니다. |
| `pick-card.js` | 복불복의 카드 뽑기입니다. 당첨 k 장을 숨긴 카드(`pickCardDeal`)를 명단 차례대로 한 사람씩 뒤집고, 당첨자가 다 나오면 끝납니다(이미 당첨된 사람은 `pickCardNextTurn` 이 건너뜀). 카드 수는 기본이 사람 수이고 당첨 수는 카드 수-1·사람 수를 넘지 않습니다(`pickCardPlan`). 판 크기에 맞춰 열 수를 고르고(`pickCardGrid`), 섞기는 카드가 가운데로 모였다 흩어지는 CSS 전환입니다. |
| `pick-slot.js` | 복불복의 슬롯 추첨입니다. 당첨자를 먼저 뽑고 릴마다 띠(`pickSlotStrip` — 지금 보이는 세 칸 + 무작위 채움 + 당첨자를 가운데로 한 세 칸)를 만들어 끝 칸까지 굴립니다. 늦게 멈추는 릴일수록 채움이 길고, 멈출 때 살짝 지나쳤다 돌아옵니다(`pickSlotEase`). |
| `pick-marble.js` | 복불복의 구슬 경주입니다. 물리 계산 없이 순위를 먼저 섞고 등수마다 도착 시각을 조금씩 늦게 준 뒤(`pickMarblePlan`), 진행도 p(u)=u+u(1-u)·Σa_k·sin(2πku+φ_k) 로 흔들어(`pickMarbleProgress`, 기울기가 늘 양수라 뒤로 가지 않음) 중간에 앞뒤가 뒤바뀌게 합니다. 길은 조종점을 Catmull-Rom 으로 이은 점 목록(`pickMarblePath`)이고, 출발 칸·결승선·도착 칸·나무·표지판을 한 SVG 로 그립니다. 30명까지입니다. |
| `pick-capsule.js` | 복불복의 캡슐 뽑기입니다. 유리 통에 참가자 색 캡슐(40개까지 그림)을 `pickPackCircle` 로 바닥부터 쌓고, 뽑을 사람을 먼저 정한 뒤 손잡이 돌기·통 흔들기·캡슐 떨어지기·결과 칸에서 캡슐 열기를 차례로 보여 줍니다. SVG 에서 transform 속성이 있는 요소엔 CSS 움직임을 걸지 않습니다. |
| `pick-dice.js` | 복불복의 주사위 굴리기입니다. 명단 차례대로 주사위(1~3개)를 굴려 합이 가장 높은(설정으로 낮은) 사람이 이기고, 맨 앞 동점이면 `pickDiceLeaders` 로 고른 사람들만 재대결합니다. 굴린 값은 굴릴 때마다 새로 뽑습니다(`pickDiceRoll`). 다음 차례로 넘어갈 때까지 busy 로 두어 같은 사람이 두 번 굴리지 않게 합니다. |
| `pick-lotto.js` | 복불복의 공 뽑기입니다. 참가자 번호(전체 명단 차례, `pickLottoNumber`)를 참가자 칸 배지(게임의 `badge`)와 공에 함께 씁니다. 공은 간단한 튕김 계산(`pickLottoStep` — 중력·바람·벽·공끼리)으로 섞이지만 뽑을 공은 먼저 정하고, 뽑힌 공은 관(`pickLottoTube`)을 따라 받침으로 굴러 나와 통에 다시 들어가지 않습니다(누를 때마다 하나씩, 다 뽑으면 다시 넣기). 60명까지입니다. |
| `pick-bomb.js` | 복불복의 폭탄 돌리기입니다. 참가자를 둥글게 앉히고, 시작하면 터지는 시각을 [가장 짧게, 가장 길게] 사이에서 숨겨 정합니다(`pickBombFuse`). 도는 동안에도 시작 단추가 '다음 사람에게'가 되도록 `goWhileBusy` 를 씁니다. 째깍 간격(`pickBombTickGap`)은 터질 때가 가까울수록 짧아지고, 터지는 순간 폭탄을 든 사람이 결과입니다. 24명까지입니다. |
| `pick-scratch.js` | 복불복의 스크래치 뽑기입니다. 참가자마다 자기 색 카드 한 장, 당첨 k 장은 섞을 때(`pickScratchDeal`) 정합니다. 은색 막은 canvas 이고 긁은 자리는 0~1 좌표 선으로 기억해 창 크기가 바뀌면 다시 그립니다. 16×16 칸(`pickScratchCover`) 가운데 55% 넘게 긁히면 막이 걷히고, 한 번 누르면 지그재그로 저절로 긁습니다(`pickScratchAutoPath`). 24명까지입니다. |
| `pick-bottle.js` | 복불복의 병 돌리기입니다. 참가자를 탁자 둘레(12시부터 시계 방향)에 앉히고, 뽑을 사람을 먼저 정한 뒤 병목이 그 자리(± 자리 간격 30%)를 가리키도록 멈출 각을 셈합니다(`pickBottleTarget`, 가리키는 자리는 `pickBottleIndexAt`). 24명까지입니다. |
| `pick-dart.js` | 복불복의 다트 추첨입니다. 과녁 칸은 룰렛 그림(`pickRouletteSlices`)을 그대로 쓰고, 뽑을 사람을 먼저 정해 그 칸 안의 한 점(`pickDartSpot` — 가운데 과녁·테두리는 피함)으로 다트가 포물선을 그리며 날아갑니다. 좌표→칸은 `pickDartIndexAt`. 꽂힌 다트는 여섯 개까지 남깁니다. 30명까지입니다. |
| `pick-treasure.js` | 복불복의 보물상자 고르기입니다. 규칙과 장수 셈은 카드 뽑기(`pickCardPlan`·`pickCardDeal`·`pickCardNextTurn`·`pickCardGrid`)를 함께 쓰고, 상자 그림(`PICK_TREASURE_CHEST`)의 뚜껑·빛·별은 transform 속성 없이 CSS 로 엽니다. 상자는 30개까지입니다. |
| `pick-croc.js` | 복불복의 악어 이빨 누르기입니다. 판을 시작할 때 물리는 이빨 하나를 무작위로 정하고, 명단 차례대로 이빨을 누르다 그 이빨을 누른 사람이 걸립니다. 이빨 수는 기본이 사람 수(`pickCrocTeethCount`, 4~16)라 모두 1/n 로 같고, 이빨은 잇몸 타원을 따라 놓되 많으면 겹치지 않게 줄입니다(`pickCrocTeeth`). |
| `pick-bingo.js` | 복불복의 빙고 추첨입니다. 참가자마다 판(`pickBingoCard` — 열마다 제 범위의 서로 다른 번호, 가운데 빈칸)을 나누고, 1~75 가운데 안 나온 번호를 하나씩 뽑아(`pickBingoDraw`) 모든 판에 표시합니다. 다 찬 줄은 `pickBingoLines`(가로·세로·대각선 12줄)로 세고, 정한 줄 수를 먼저 채운 사람(동시면 모두)이 당첨입니다. 30명까지입니다. |
| `pick-lots.js` | 복불복의 제비뽑기입니다. 유리그릇 안 참가자 쪽지(`pickPackCircle` 로 쌓음) 가운데 뽑을 사람을 먼저 정해, 그 쪽지가 그릇 밖으로 올라와 앞으로 날아오며 펼쳐집니다. 뽑은 쪽지는 탁자 위 텐트(`PICK_LOTS_TENTS`, 최근 여덟 장)로 남고 다시 넣지 않아 누를 때마다 남은 사람에서 한 명씩입니다. 60명까지입니다. |
| `pick-coin.js` | 복불복의 동전 던지기입니다. 참가자를 앞면·뒷면 편으로 나누고(`pickCoinSplit` 무작위 반반, 이름을 누르면 편 옮김 — 화면에만), 나올 면을 먼저 정한 뒤 그 면으로 떨어지도록 도는 각을 셉니다(`pickCoinSpin`, 면 판정은 `pickCoinSideAt`). 나온 면의 편 모두가 결과입니다. |
| `pick-strings.js` | 복불복의 끈 뽑기입니다. 규칙은 카드 뽑기와 같아(당첨 k 개를 나눌 때 정하고 명단 차례대로 한 번씩, `pickCardPlan`·`pickCardDeal`·`pickCardNextTurn` 공용) 끈을 당기면 당첨 끈은 상자 위 선물 창이 올라오고, 꽝이면 끈이 되돌아가며 당긴 사람 이름이 남습니다. 끈 안쪽 g 엔 transform 속성이 없어 CSS 로 당깁니다. 16개까지입니다. |
| `pick-balloon.js` | 복불복의 풍선 터뜨리기입니다. 참가자마다 자기 색 풍선 하나, 당첨 쪽지 k 개는 채울 때(`pickBalloonDeal`) 정합니다. 누르면 떨다가 터지며(조각·색종이는 CSS 변수로 날림) 쪽지가 보이고, 당첨이 다 나오면 끝입니다. 24명까지입니다. |
| `pick-pinball.js` | 복불복의 핀볼 추첨입니다. 구슬 자리를 반 칸 단위로 세어 줄마다 ±1 씩 움직이고, 줄 수(`pickPinballRows`)를 n-1 과 홀짝이 같게 잡아 어느 칸이든 닿게 합니다. 뽑을 칸을 먼저 정해 그 칸에 닿는 걸음을 무작위로 짜고(`pickPinballPath` — 벽 밖으로 안 나가고 끝에 닿을 수 있는 걸음만), 구슬이 핀마다 톡톡 튀며 그 길을 따라갑니다. 16명까지입니다. |
...
| `diary.js` | 일기장 문서(`.diary`)입니다. 파일 하나가 일기장 한 권이고 속은 무압축 ZIP(`diary.json` + `assets/` 사진)이라, 사진이 쌓여도 JSON 이 base64 로 불어나지 않습니다. ZIP 쓰기·읽기(DEFLATE 항목은 `DecompressionStream`)를 직접 해서 JSZip 없이 새 문서 뼈대를 동기로 만듭니다. 월 달력(일기 있는 날 점)·이번 달 목록·전날/다음날 이동, 날짜마다 글·제목, 줄 무늬 14종(줄 공책·두 줄·세 줄·점선·목록·모눈·세로줄·점·십자·오선·사선 격자·빈 종이·그림일기·원고지, 간격 3단계 — textarea 배경 그라디언트라 글줄과 딱 맞음, 그림일기는 위 그림 칸(사진·펜 그리기 — 획을 벡터로 저장) + 아래 원고지, 원고지는 textarea 를 숨은 입력칸으로 두고 `diaryGenkoLayout` 규칙대로 칸마다 글자를 그림, 한 줄 칸 수는 자동·8~24칸 — 칸 줄은 `diaryGenkoGrid` 한 곳에서 잼), 이 날·이번 달·전체 인쇄(인쇄 전용 층·A4 폭), 영어 화면(`translateTree`+`diaryT`/`diaryTf`, 짧은 이름은 파일 안 영어 표), 글꼴 11종(시스템 한글 글꼴 4 + 앱에 담은 손글씨 7 — 고를 때만 `MNLazy` 로 읽어 `FontFace` 로 올림), 날씨·기분(달력에 그림 — `.ui-keep-symbols` 안), 스티커 돌리기·뒤집기·앞뒤 순서(우클릭 `MNContextMenu`)·여러 장 고르기(Shift/Ctrl+클릭·Ctrl+끌기 네모·Ctrl+A, 순서는 `diaryReorder`), 배경 그림(채우기·맞추기·반복, 종이색으로 흐리게), 일기장 전체/그 날짜만 꾸미기, 종이 폭 비율로 자리를 기억하는 사진 스티커(끌기·모서리 크기·방향키·Delete, 종이에 떨어뜨리기·붙여넣기), 되돌리기·복구본·통합 검색을 담당합니다. | `documents.js`, `file-loaders.js`, `code-viewer.js`(`saveTextDoc` 이진 저장), `history.js`, `spellcheck.js`, `python-editor.js`(글칸 우클릭 메뉴), `context-menu.js`(스티커 메뉴), `app.js`(파일 드롭 오버레이 예외), `tests/diary.test.js` |
| `trip.js` | 여행일지 문서(`.trip`)의 모델과 형식입니다. 파일 하나가 여행 한 건이고 속은 일기장과 같은 무압축 ZIP(`trip.json` + `assets/*`)이라, ZIP·해시·사진 줄이기·스티커 정규화는 `diary.js` 것을 그대로 부릅니다. 낱말은 `TRIP_WORDS`/`TRIP_WORDS_EN` 한 표에 모으고(`{n}`·`{sum}` 이 있는 줄은 낱말이 아니라 틀이라 `tripWordf` 로 채웁니다), 장소의 종류는 **7가지**(`TRIP_SPOT_KINDS`)입니다. **모르는 종류도 버리지 않고** 고르개 맨 아래에 글자 그대로 보여 줍니다(일기장이 모르는 `art` 를 버리는 것과 반대 규칙). 체험학습·답사 갈래(`purpose`)는 2026-09-23 걷어냈고, 옛 파일의 `purpose`·`prompts`·`fields` 는 읽을 때 버립니다. 하루(day)는 일기장의 하루와 같은 모양에 장소(spots)가 얹힌 것이고, 장소를 스티커와 따로 두는 까닭은 지도·일정·경비 세 곳이 같은 자료를 읽기 때문입니다. 국내·해외 판정은 `tripIsDomestic` 한 곳에서만 합니다(좌표가 하나도 없으면 국내로 봅니다). 설계는 `docs/여행일지-설계.md`. | `diary.js`(종이 엔진 `mountDiaryPaper`·ZIP·스티커), `korea-coords.js`, `documents.js`, `file-loaders.js`, `tests/trip.test.js` |
...
| `music-pitch.js` | 마이크 음 확인(`MNMusicPitch`)입니다. 따라치기의 **🎤 따라 부르기**가 씁니다. 마이크 소리 한 조각(약 40ms)에서 YIN 알고리즘으로 음높이를 찾고(`detect`), 조각을 이어 보며 “한 음을 냈다”는 사건을 만듭니다(`createTracker` — 같은 반음에 머문 시간으로 판정, 길게 끄는 음은 한 번만, 같은 음을 다시 내려면 잠깐 쉬어야 함, 반음 사이 ±45센트 밖은 무시). 머무는 시간은 함수로 받아 편집기가 맞는 음은 짧게(120ms)·틀린 음은 길게(450ms) 줍니다 — 노래는 음을 찾아 미끄러져 들어가서 같은 기준이면 스친 음이 모두 실수가 됩니다. `createMic`은 메아리 제거·잡음 억제·자동 음량을 끈 채 마이크를 열고, 앱이 정답 음을 들려주는 동안 `mute(ms)`로 귀를 막아 스피커 소리를 채점하지 않습니다. 채점(옥타브 통과·틀리면 진도 멈춤)은 편집기의 `practicePress(pc, "mic")`가 그대로 맡고, 목소리는 화음을 낼 수 없어 화음 중 한 음만 맞아도 넘어갑니다. | `music-editor.js`, `tests/music-pitch.test.js` |
