...
| `db-client.js` | EXE 전용 MySQL 접속 문서(`.dbconn`)입니다. 호스트·포트·데이터베이스·계정 입력과 연결, 데이터베이스 전환·테이블 목록·컬럼 정의·미리보기, SQL 편집기 실행·취소·제한 시간, 결과 표와 CSV·표 편집기 내보내기를 담당합니다. 실행 대상은 선택 영역 → 커서가 놓인 문장 순으로 정해지고 편집기 전체는 `Ctrl+Shift+Enter`나 `전체 실행` 버튼으로만 나갑니다. 여러 결과 집합은 탭으로 모두 보여 주고 걸린 시간을 함께 적습니다. 셀을 누르면 값 전체를 보는 패널이 열리며 서버에서 잘라 온 값은 그렇다고 밝힙니다. 접속마다 표시색을 골라 운영 DB 를 구분할 수 있습니다. 스키마 트리에서 테이블을 펼쳐 컬럼을 보고 이름을 편집기에 바로 넣을 수 있으며(평범하지 않은 이름은 역따옴표로 감쌉니다), `DDL 보기`로 `SHOW CREATE TABLE` 결과를 봅니다. `Ctrl+/` 로 줄 주석을 토글하고 `Ctrl+Space` 로 테이블·컬럼·키워드를 자동완성합니다(`FROM` 뒤에는 테이블만, `별칭.` 뒤에는 그 테이블 컬럼만). `실행 계획` 은 `EXPLAIN` 을 붙여 실행하고, 1000행을 넘는 결과는 `더 보기` 로 이어 받으며, `이력` 은 접속별 최근 쿼리 50건을 보여 줍니다. 결과 헤더를 누르면 클라이언트에서 행을 다시 늘어놓지 않고 문장의 `ORDER BY` 를 고쳐 다시 실행합니다(잘린 결과를 정렬해 보여 주지 않기 위해서이며, 편집기 내용은 그대로 둡니다). 저장은 `run-save` 클래스로 앱의 전역 `Ctrl+S` 경로를 그대로 씁니다. 스키마 패널과 편집기 사이 세로 분할선으로 폭을, 편집기와 결과 사이 가로 분할선으로 높이를 조절하고 더블클릭으로 기본값에 되돌립니다(둘 다 브라우저에 저장). 접속은 읽기 전용이 기본이고 쓰기 허용은 연결할 때만 켤 수 있으며, `DROP`·`TRUNCATE`·`WHERE` 없는 `DELETE`/`UPDATE`는 실행 전에 확인을 받습니다. 접속 정보와 SQL 만 파일에 저장하고 비밀번호는 저장하지 않습니다. | `code-viewer.js`(SQL 강조), `table-export.js`, `file-loaders.js`, `desktop/db_worker.py`, `desktop/launcher.cs`, `tests/db-client.test.js`, `docs/DB클라이언트-설계.md` |
