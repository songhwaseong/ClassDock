...
| `concept-doc.js` | 개념 관계도 문서(`.concept`)의 카드·연결선 모델과 편집 화면입니다. 원인→결과·상위→하위·비교·근거·관련 관계, **5가지 자동 배치**(위→아래 가계도·선택/시작 카드 중심 방사형·원형·왼쪽→오른쪽·격자형), 3단계 간격과 정렬 후 화면 맞춤(`conceptFitZoom`), 30,000px 동적 캔버스와 가장자리 자동 이동, 스크롤 대신 무대 변환으로 사방 자유 이동(`conceptClampPan`)하는 손바닥·터치 끌기, 커서 중심 `Ctrl+휠`·두 손가락 핀치 확대/축소(`conceptZoomPan`), 휠·`Shift+휠`·방향키 이동과 `Home` 화면 맞춤, 카드 한 번 클릭 큰 보기, 발표 순서 편집, `Space` 단계 공개 전개 발표와 5가지 효과, 큰 카드 발표·인쇄·되돌리기·자동 복구를 담당합니다. 배치 방식과 간격은 문서에 저장하며 예전 자동 정렬 문서는 흐름형으로 호환합니다. | `timeline.js`, `documents.js`, `file-loaders.js`, `history.js`, `code-viewer.js`, `tests/concept-doc.test.js` |
...
| `music-model.js` | 악보 문서(`.msheet`)의 순수 모델입니다. 마디·음표·쉼표 구조, `.msheet` JSON 읽기·쓰기(같은 모델은 항상 같은 바이트), 4분음표=480틱 정수 길이 계산과 점음표, 마디 채움 검사, 음높이 → MIDI 번호 → 주파수 변환, 조표와 비교해 임시표를 그릴지 정하는 VexFlow 변환, 재생·저장이 함께 쓰는 초 단위 타임라인, 편집에 쓰는 오선 자리 계산(줄 값 ↔ 음높이, 흰건반 한 음씩 올리고 내리기, 음역 검사), 조표를 바꿀 때 임시표 없던 음만 새 조표로 맞추기, 화면 폭과 마디별 음표 수로 줄을 나누는 폭 배분, 따라치기(음 맞추기)가 쓸 채점 순서 만들기(`musicPracticeSteps` — 타임라인을 동시에 울리는 음 묶음으로 잘라 옥타브를 뺀 음이름과 쉼표 자동 통과 표시를 붙인다), 음감 테스트의 문제 만들기·채점·성적표(`musicEarQuestions`·`musicEarJudge`·`musicEarSummary`·`musicEarDistractor` — 단계별 음역에서 무작위로 뽑되 같은 음이 연달아 나오지 않게 하고, 문제 사이 간섭음과 혼동한 음 짝까지 셈한다)를 담당합니다. 음높이는 VexFlow 표기가 아니라 `{step, octave, alter}`로 저장해 파일 형식이 그리기 라이브러리에 묶이지 않게 합니다. DOM·오디오·VexFlow를 참조하지 않아 브라우저 없이 검증합니다. | `music-audio.js`, `docs/악보-설계.md`, `tests/music-model.test.js` |
...
| `music-editor.js` | 악보 문서(`.msheet`)의 화면과 편집입니다. 파일 열기·저장, VexFlow 조판, 오선 클릭 입력, 음표 좌우 미세 이동, 계이름, 확대·축소·끌어 이동, 우클릭 메뉴, 되돌리기, 전체·구간 재생, 자판을 건반 삼아 음을 따라 누르는 따라치기(옥타브는 통과·쉼표는 자동·`event.code`로 읽어 한글 입력 상태와 무관), 소리만 듣고 음이름을 맞히는 음감 테스트 붙이기(`MNMusicEarTest` — 단계·문항 수·기준음 고르기와 자판·도레미 버튼·MIDI 건반을 답으로 돌리는 배선), WAV와 MusicXML 내보내기, 인쇄를 담당합니다. 따라치기 중에는 악보를 한 글자도 고치지 않고 표시만 바꾸며 편집 입력 경로(오선 클릭·드래그·우클릭·단축키·MIDI)를 모두 채점으로 돌리거나 잠급니다. VexFlow는 악보를 열 때 `MNLazy`로 불러오고, 그리기에 실패해도 재생·저장은 그대로 됩니다. 재생 중에는 편집기에 `.is-running`을 붙여 대기 화면이 뜨지 않게 합니다. | `music-model.js`, `music-xml.js`, `music-audio.js`, `music-eartest.js`, `lazy.js`, `history.js`, `documents.js`, `file-loaders.js`, `code-viewer.js`, `vendor/vexflow-bravura.min.js`, `docs/악보-설계.md`, `tests/music-editor.test.js` |
