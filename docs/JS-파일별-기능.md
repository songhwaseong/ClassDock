...
| `music-editor.js` | 악보 문서(`.msheet`)의 화면과 편집입니다. 파일 열기·저장, VexFlow 조판, 오선 클릭 입력, 음표 좌우 미세 이동, 계이름, 확대·축소·끌어 이동, 우클릭 메뉴, 되돌리기, 전체·구간 재생, 자판을 건반 삼아 음을 따라 누르는 따라치기(옥타브는 통과·쉼표는 자동·`event.code`로 읽어 한글 입력 상태와 무관), WAV와 MusicXML 내보내기, 인쇄를 담당합니다. 따라치기 중에는 악보를 한 글자도 고치지 않고 표시만 바꾸며 편집 입력 경로(오선 클릭·드래그·우클릭·단축키·MIDI)를 모두 채점으로 돌리거나 잠급니다. VexFlow는 악보를 열 때 `MNLazy`로 불러오고, 그리기에 실패해도 재생·저장은 그대로 됩니다. 재생 중에는 편집기에 `.is-running`을 붙여 대기 화면이 뜨지 않게 합니다. | `music-model.js`, `music-xml.js`, `music-audio.js`, `lazy.js`, `history.js`, `documents.js`, `file-loaders.js`, `code-viewer.js`, `vendor/vexflow-bravura.min.js`, `docs/악보-설계.md`, `tests/music-editor.test.js` |
