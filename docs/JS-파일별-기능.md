...
| `concept-doc.js` | 개념 관계도 문서(`.concept`)의 카드·연결선 모델과 편집 화면입니다. 원인→결과·상위→하위·비교·근거·관련 관계, **30,000px 범위 동적 캔버스 확장**과 가장자리 끌기 자동 스크롤, 커서 중심 `Ctrl+휠`·도구막대 35~200% 확대/축소(`conceptZoomScroll`), 자유 배치·방향 관계 자동 정렬, 카드 한 번 클릭 큰 보기, **발표 순서 편집**(끌기·화살표·관계 방향 자동 순서·화면 위치순), `Space` 단계 공개 전개 발표와 5가지 효과, 기존 큰 카드 발표·인쇄, 되돌리기와 자동 복구·저장을 담당합니다. 애니메이션은 동작 줄이기 환경설정을 따르고, 짧은 클릭과 끌기 이동·두 번 클릭 편집을 구분합니다. 통합검색에는 사진 base64를 제외한 카드와 관계 글자만 제공합니다. | `timeline.js`, `documents.js`, `file-loaders.js`, `history.js`, `code-viewer.js`, `tests/concept-doc.test.js` |
