...
| `docx-editor.js` | Word(.docx) 문단·표 편집 화면(`MNDocxEditor`)입니다. 먼저 docx-preview가 그린 문단과 XML 문단을 글자·순서로 대조해 **보이는 자리에서 바로 편집**하고, 표 병합·필드 등으로 대조가 어긋나면 저장하지 않는 임시 북마크로 화면 위치를 정확히 다시 찾습니다. 문단을 선택하면 글꼴·크기·굵게 도구가, 표 셀을 선택하면 행·열 추가/삭제와 가로·세로 정렬, 배경색, 테두리, 열 너비, 행 높이 도구가 나타납니다. 문단 글자를 먼저 XML에 반영한 뒤 문서를 다시 그리며, 병합·중첩 표의 위험한 구조·너비 편집은 기존 구조 보존을 위해 제한합니다. XML 버전 번호를 편집 기록에 함께 넣어 모든 서식·표 변경도 Ctrl+Z/Ctrl+Y로 되돌립니다. 무엇을 어떻게 되쓸지는 `MNOfficeReplace`의 순수 함수(`officeParagraphEditPlan`, `officeParagraphFormatEdit`, `officeTableStructureEdit`, `officeTableFormatEdit`)가 정합니다. | `office-replace.js`, `office-doc-viewers.js`, `docs/워드-문단편집-설계.md`, `tests/office-replace.test.js` |
