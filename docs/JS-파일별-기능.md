...
| `diary.js` | 일기장 문서(`.diary`)입니다. 파일 하나가 일기장 한 권이고 속은 무압축 ZIP(`diary.json` + `assets/` 사진)이라, 사진이 쌓여도 JSON 이 base64 로 불어나지 않습니다. ZIP 쓰기·읽기(DEFLATE 항목은 `DecompressionStream`)를 직접 해서 JSZip 없이 새 문서 뼈대를 동기로 만듭니다. 월 달력(일기 있는 날 점)·이번 달 목록·전날/다음날 이동, 날짜마다 글·제목, 줄 무늬 6종(줄 공책·모눈·점·빈 종이·그림일기·원고지, 간격 3단계 — textarea 배경 그라디언트라 글줄과 딱 맞음, 그림일기는 위 그림 칸(사진·펜 그리기 — 획을 벡터로 저장) + 아래 원고지, 원고지는 textarea 를 숨은 입력칸으로 두고 `diaryGenkoLayout` 규칙대로 칸마다 글자를 그림, 한 줄 칸 수는 자동·8~24칸 — 칸 줄은 `diaryGenkoGrid` 한 곳에서 잼), 이 날·이번 달·전체 인쇄(인쇄 전용 층·A4 폭), 영어 화면(`translateTree`+`diaryT`/`diaryTf`, 짧은 이름은 파일 안 영어 표), 글꼴 6종(시스템 한글 글꼴 4 + 앱에 담은 손글씨 2 — 고를 때만 `MNLazy` 로 읽어 `FontFace` 로 올림), 날씨·기분(달력에 그림 — `.ui-keep-symbols` 안), 스티커 돌리기·뒤집기·앞뒤 순서(우클릭 `MNContextMenu`)·여러 장 고르기(Shift/Ctrl+클릭·Ctrl+끌기 네모·Ctrl+A, 순서는 `diaryReorder`), 배경 그림(채우기·맞추기·반복, 종이색으로 흐리게), 일기장 전체/그 날짜만 꾸미기, 종이 폭 비율로 자리를 기억하는 사진 스티커(끌기·모서리 크기·방향키·Delete, 종이에 떨어뜨리기·붙여넣기), 되돌리기·복구본·통합 검색을 담당합니다. | `documents.js`, `file-loaders.js`, `code-viewer.js`(`saveTextDoc` 이진 저장), `history.js`, `spellcheck.js`, `python-editor.js`(글칸 우클릭 메뉴), `context-menu.js`(스티커 메뉴), `app.js`(파일 드롭 오버레이 예외), `tests/diary.test.js` |
