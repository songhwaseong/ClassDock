...
| `bracket.js` | 대진표 문서(`.bracket`)의 토너먼트 모델과 편집 화면입니다. 나무는 늘 2의 거듭제곱 칸(2~128)이고 빈 칸은 부전승이며, 1회전 자리는 `slots`(씨드 차례 `bracketSeedOrder` 로 앉혀 부전승이 고루 흩어짐), 경기 결과는 `results["회전:번호"]` 에 두 참가자(a·b)와 함께 담아 앞 경기가 바뀌면 뒤 결과가 저절로 무효가 됩니다(`bracketResolve`·`bracketPrune`). 모양 6가지(양쪽→가운데·왼→오·오→왼·아래→위·위→아래·위아래→가운데)는 `bracketGeometry` 한 곳에서 같은 나무를 줄(lane)×자리(cross)로 계산한 뒤 좌표만 바꿉니다. 이긴 쪽을 누르면 선이 차오르고 카드가 다음 칸으로 날아가는 애니메이션(Web Animations), 우승 왕관·색종이, 정한 경기를 1회전부터 되살리는 **다시 보기**, 점수 입력(높은/낮은 점수 승), 배경 템플릿 10종+내 사진(`[data-br-theme]` CSS 변수 — PNG 는 `BRACKET_THEMES.png` 색으로 근사), 꺾은선/곡선·카드 크기, 확대·밀기, 첫 칸 끌어 자리 바꾸기, 캔버스로 직접 그리는 PNG 저장·칠판 보내기를 담당합니다. 사진 줄이기·창(`tierPrepareImage`·`tierModal`)은 `tier-list.js` 것을 함께 씁니다. 되돌리기 기록엔 사진 대신 짧은 열쇠만 담습니다. | `tier-list.js`, `documents.js`, `file-loaders.js`, `history.js`, `code-viewer.js` |
