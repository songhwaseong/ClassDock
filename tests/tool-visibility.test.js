...
const mapSource = ["map-viewer.js", "jeju-bus-map.js"].map(file => fs.readFileSync(path.join(__dirname, "../src/js", file), "utf8")).join("\n");
...
test("도구 레지스트리는 전체 화면의 선택 도구를 담고 필수 버튼(실행·저장)은 제외한다", () => {
  ...
  assert.equal(TOGGLEABLE_TOOLS.length, 181);
  assert.deepEqual(
    ...
    { header:12, py:15, javascript:2, java:12, notebook:7, image:12, whiteboard:37, map:31, music:53 }
  );
