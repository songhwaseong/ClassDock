 ...
 *   · 원본에 직접 쓸 수 있으면 "원본 · 저장됨"
 *   · 사본으로만 저장되면 "사본으로 저장"(예전에는 배지가 아예 없어 "저장할 수 없는 문서"와 구분이 안 됐다)
...
const badge = (page) => page.locator("#saveStatusBadge");

test("파일 입력으로 연 편집 가능한 문서는 '사본으로 저장'으로 안내한다", async ({ page }) => {
  await boot(page);
...
test("원본에 바로 쓰는 문서는 저장 위치와 상태를 함께 안내한다", async ({ page }) => {
  ...
  await expect(badge(page)).toHaveText("원본 · 저장됨");
...
test("배지 안내는 화면 낭독기에도 같은 내용으로 전달된다", async ({ page }) => {
  ...
  expect(label).toContain("사본으로 저장");
